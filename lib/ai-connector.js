...
const DEFAULT_AI_CONFIG = {
  ...
  dataLevel: 'categories_only', // 'categories_only' | 'include_site_name'
};

// Load AI config from storage (minus API key — that's session-only).
// Returns camelCase throughout so callers can't reintroduce the old
// `data_level` vs `dataLevel` footgun.
export const getAIConfig = async () => {
  ...
  return {
    ...
    dataLevel: stored.ai_data_level,
  };
};
...
// Extract text from an OpenAI-compatible response, including Anthropic-style
// `content: [{type:'text', text:'...'}]` arrays that some providers return
// through OpenAI-compatible shims.
...
//
// Privacy: in 'categories_only' mode we send type, topicKeywords, lang, and
// coarse flags (hasVideo/hasCode) — but NOT site name, section, or the
// description snippet. In 'include_site_name' mode we additionally include
// site/section and the description snippet (still the page's own public
// meta tag — never URLs, titles, or user input).
const sanitizeSignals = (signals, dataLevel) => {
  const includeSite = dataLevel === 'include_site_name';
  return signals.map(s => {
    const safe = {
      type: s.signal.type,
      topicKeywords: s.signal.topicKeywords,
      lang: s.signal.lang,
      hasVideo: s.signal.hasVideo,
      hasCode: s.signal.hasCode,
      // Local-only metadata (not from the page content)
      age: s.age,
      visits: s.visits,
      lastSeen: s.lastSeen,
      memory: s.memory,
