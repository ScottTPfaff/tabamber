...
let state = {
  ...
  groupsArchived: 0,              // total stale groups archived to bookmarks
...
const init = async () => {
  ...
  if (stored[STORAGE_KEY]) {
    ...
    // Carry previous cumulative uptime AND add the elapsed time of the
    // previous session (from prev.startedAt until we were torn down). Without
    // this, totalUptimeMs stays stuck at whatever was last persisted.
    const prevElapsed = prev.startedAt ? Math.max(0, Date.now() - prev.startedAt) : 0;
    state.totalUptimeMs = (prev.totalUptimeMs || 0) + prevElapsed;
    ...
    state.groupsArchived = prev.groupsArchived || 0;
