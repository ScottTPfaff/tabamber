...
// Debounce: don't write to storage on every single log — batch every 500ms
let flushTimer = null;
...
const write = async (level, message, context = {}) => {
  ...
  // Persist (debounced — batch writes every 500ms)
  scheduleFlush();
  ...
  // Webhook (fire and forget). `keepalive` is a fetch-from-page option and
  // has no useful effect in a service worker context, so we drop it.
  if (config.webhook) {
    try {
