...
//
// We count how many *feature families* (site, type, section, keywords) each
// pair actually shared. A same-site match alone is not enough to cluster —
// two unrelated GitHub repos shouldn't be grouped together just because they
// both set og:site_name = "GitHub". We require at least two corroborating
// signals, otherwise we cap the returned similarity below the default
...
function similarity(a, b) {
  ...
  const aKw = Array.isArray(a.topicKeywords) ? a.topicKeywords : [];
  const bKw = Array.isArray(b.topicKeywords) ? b.topicKeywords : [];
  if (aKw.length > 0 && bKw.length > 0) {
    const setA = new Set(aKw);
  ...
  // Require corroboration: a single matching feature family is too weak to
  // cluster on its own. Pin it below the default 0.35 threshold.
  if (shared < 2) return Math.min(raw, 0.25);
  return raw;
}
