...
const showChromeNotification = (title, message, priority = 'normal') => {
  ...
  // Simple amber circle as base64 SVG
  ...
  // Unique ID so we only handle OUR notifications in onClick
  const notifId = `tabamber-${Date.now()}-${Math.random().toString(36).slice(2, 6)}`;
...
// Handle notification click → open diagnostics (only for OUR notifications).
// Guarded so this module can be imported from contexts where the
// notifications API isn't available (e.g., a popup that only wants the
// config helpers) without throwing at import time.
if (typeof chrome !== 'undefined'
    && chrome.notifications
    && chrome.notifications.onClicked) {
  chrome.notifications.onClicked.addListener((notifId) => {
    if (!notifId || !notifId.startsWith('tabamber-')) return;
